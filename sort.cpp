...
void initial_split(FILE * file_in, int block_size, int* count_pages, vector<FILE*> *pages){
    ...
    for(int i = 0; i < (*count_pages); i++)
        rewind((*pages)[i]);    
    // int tmp;
    // fscanf((*pages)[0], "%d", &tmp);
    // cout << tmp << endl;
    cout << "# of lines " << cc << endl;  
...
void merge_file(FILE *file_out, int block_size, int sum_pages, vector<FILE*>*pages){
    ...
    while(count_pages != sum_pages){
        ...
        //cout << tmp << endl;
        if(feof((*pages)[standard_pages]) != 0){ //the file is empty and count the # of empty file
            count_pages ++;
            arr[standard_pages] = INT_MAX;
            cout << "out_temp_" << standard_pages << ".txt has been sorted finish." << endl;
        }
        else {
            fprintf(file_out, "%d\n", standard); //write file
        }
    ...
    for(int i = 0; i < sum_pages; i++){
        if(arr[i] != INT_MAX){
            fprintf(file_out, "the last one %d\n", arr[i]);
        }
    }
    // for(int i = 0; i < sum_pages; i++){
    //     string title = "out_temp_" + to_string(i) + ".txt";
